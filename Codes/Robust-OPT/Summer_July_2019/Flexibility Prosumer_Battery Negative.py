...
for i in range(iterations):
    ...
    if i%2 == 0:
        binary_battery_variable = switching_operation_battery[0] # even i then battery_variable is 0
    else:
    ...
    if binary_battery_variable == 1:
        ...
        model_flex.addConstr(P_bat_charging_PV_flex == 0.25 * (PV_ninety_percent_quantiles[i] + PV_ten_percent_quantiles[i])/2)#R3
    else:
    ...
    BAT_FLEX_OBJ = (P_grid_import_flex )* epex_spot_import_price_flex[i]*time_res  - (
    ...
    if model_flex.status == gp.GRB.OPTIMAL:
        ...
        P_bat_charging_PV_opt_flex[i] = P_bat_charging_PV_flex.x - P_pv_positive_flex[i]
...
for i in range(iterations):
 ...
 cost_per_cycle = 0.01 # 10 cents is the cost per battery charging/discharging
