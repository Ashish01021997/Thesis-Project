...
for i in range(iterations):
    ...
    model_flex.addConstr(P_bat_export <= abs(P_bat_positive_flex[i])) #R10
...
plt.title('EMS Optimization with positive Battery flexibility provision')
plt.xlabel('X')
plt.ylabel('y')
plt.legend()
plt.show()
...
for i in range(iterations):
 ...
 def Bat_Pos_Flex_Price():
     ...
     savings_bat_pos_flex = -abs(
         (P_bat_discharging_load_opt_flex[i] - P_bat_discharging_load_opt[i]) * epex_spot_export_price_flex[
