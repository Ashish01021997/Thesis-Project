...
C_bat = 4600 # in Wh
Panel_area = 2.5 # 2msq panel area
counter = 0
SOC_min = 0.05
SOC_max = 0.95
...
time_scale = np.arange(1, 6, 1)

n_steps = 24 # 24 hours in a day

iterations = len(time_scale)*n_steps

Load_hour_forecasts_lower = np.zeros(iterations)
Load_hour_forecasts_upper = np.zeros(iterations)
PV_hour_forecasts_lower = np.zeros(iterations)
PV_hour_forecasts_upper = np.zeros(iterations)
...
Load_forecaster = Load_forecasting.Load_forecast()

EM_Price = EM_Price_forecaster.y_pred

for i in range(iterations):
    PV_hour_forecasts_upper[i] = PV_forecaster.upper_bound[i]
    PV_hour_forecasts_lower[i] = PV_forecaster.lower_bound[i]
    Load_hour_forecasts_lower[i] = Load_forecaster.lower_bound_hour[i]
    Load_hour_forecasts_upper[i] = Load_forecaster.upper_bound_hour[i]

offset = np.ones(len(time_scale))*20.0 # offset is 20 EUR/MWh
...
bat_operational_cost = 50



 # 60 minutes in 24 hours so 1440 iterations
...
for i in range(iterations):
    ...
    model.addConstr(P_PV_export_grid + P_bat_charging_PV <= P_PV_gen)

    model.addConstr(P_PV_gen >= PV_hour_forecasts_lower[i])
    model.addConstr(P_PV_gen <= PV_hour_forecasts_upper[i])
    model.addConstr(P_load >= Load_hour_forecasts_lower[i])
    model.addConstr(P_load <= Load_hour_forecasts_upper[i])

      #model.addConstr(E_bat_discharging_grid <= 0.5*E_bat_discharging) # load handling is essential for battery than grid stability
      #model.addConstr(E_bat_charging_PV <= E_PV_export_grid) # battery charging is equally or less important than grid stability
    model.addConstr(
...
for i in range(iterations):
  ...
  P_PV_export_grid_flex = model_flex.addVar(lb=0, ub=max(PV_hour_forecasts_upper), vtype=gp.GRB.CONTINUOUS,
  ...
  model.addConstr(P_PV_gen_flex >= PV_hour_forecasts_lower[i])
  model.addConstr(P_PV_gen_flex <= PV_hour_forecasts_upper[i])
  model.addConstr(P_load_flex >= Load_hour_forecasts_lower[i])
  model.addConstr(P_load_flex <= Load_hour_forecasts_upper[i])
  ...
  model_flex.addConstr(FLEX_OBJ >= 0)

  model_flex.setObjective(FLEX_OBJ,
                           gp.GRB.MINIMIZE)
...
plt.plot(np.arange(1, 26, 1), E_bat_profile_flex, marker='*', label='E_bat_flex_optimum_flow')
plt.plot(np.arange(1, 26, 1), P_grid_profile_flex, marker='.', label='P_grid_flex_optimum_flow')
plt.plot(np.arange(1, 26, 1), P_grid_import_opt_flex, marker='v', label='P_grid_flex_import_optimum_flow')
plt.plot(np.arange(1, 26, 1), P_PV_export_grid_opt_flex, marker='o', label='PV_export_flex_grid_optimum_flow')
plt.show()

plt.plot(np.arange(1, 26, 1), E_bat_flex, label = 'E_bat_flex' )
plt.plot(np.arange(1, 26, 1), P_grid_flex, label = 'P_grid_flex')
