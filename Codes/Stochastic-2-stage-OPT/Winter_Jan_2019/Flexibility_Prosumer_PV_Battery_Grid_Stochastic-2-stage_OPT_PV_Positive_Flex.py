...
EM_Price = pd.read_excel('Electricity_Market_Price.xlsx', sheet_name= 'Electricity_Market_Price_Jan')['Preis (EUR/MWh, EUR/tCO2)'] # for the year 2015 - LINK : https://ember-climate.org/data-catalogue/european-wholesale-electricity-price-data/
...
bat_operational_cost = 0.1 # 10 cent/NO_OF_CHARGING/DISCHARGING_CYCLES
...
for i in range(iterations):
   ...
   if i == 48:  # 12:00  PM PV +ve  Flexibility Check
   ...
   epex_spot_import_price_flex[i] = (EM_Price_15_min_res[i] + offset[i])

   epex_spot_export_price_flex[i] = (EM_Price_15_min_res[i] - offset[i])
   ...
   for j in range(number_of_scenarios_PV):
    ...
    for k in range(number_of_scenarios_Load):
      ...
      OBJECTIVE_FUNCTION = (battery_cycles * bat_operational_cost) + (
                  ((P_grid_import_flex) * epex_spot_import_price_flex[i] - (P_PV_export_grid_flex) * epex_spot_export_price_flex[
                      i]) * time_res)
      ...
      if model_flex.status == gp.GRB.OPTIMAL:
        ...
        P_PV_export_grid_scenario_profile[l] = -abs(P_PV_export_grid_flex.x + P_pv_positive_flex[i])
        P_bat_charging_PV_scenario_opt[l] = P_bat_charging_PV_flex.x
        P_bat_charging_grid_scenario_opt[l] = P_bat_charging_grid_flex.x
        P_bat_discharging_grid_scenario_opt[l] = -P_bat_discharging_grid_flex.x
        ...
        P_bat_charging_scenario_opt[l] = P_bat_charging_PV_flex.x + P_pv_negative_flex[i] + P_bat_charging_grid_flex.x - P_bat_positive_flex[i] - P_pv_positive_flex[i] - P_bat_export.x
...
for i in range(iterations):
 ...
 cost_per_cycle = 0.5  # 10 or 0.5 euro cents is the cost per battery charging/discharging cycle
 ...
 def PV_Pos_Flex_Price():
        ...
        profit = (P_bat_charging_PV_opt_flex[i] - P_bat_export_opt_flex[i] - P_bat_charging_PV_opt[i]) * epex_spot_import_price_flex[
            i] * time_res  # Battery charging from PV at grid import price

        losses = (P_PV_export_grid_opt[i] - P_PV_export_grid_opt_flex[i]) * epex_spot_export_price_flex[i] * time_res
