...
for i in range(iterations):
   ...
   for j in range(number_of_scenarios_PV):
    ...
    for k in range(number_of_scenarios_Load):
      ...
      model_flex.addConstr(P_bat_export <= abs(P_bat_negative_flex[i])) #5
      ...
      if model_flex.status == gp.GRB.OPTIMAL:
        ...
        P_PV_export_grid_scenario_profile[l] = -(P_PV_export_grid_flex.x + P_pv_positive_flex[i])
        P_bat_charging_PV_scenario_opt[l] = P_bat_charging_PV_flex.x - P_pv_positive_flex[i]
        ...
        P_PV_export_grid_opt_flex[i] += P_PV_export_grid_scenario_profile[l] * average_stochastic_probabilities[l]
...
date_rng = pd.date_range(start='2019-01-25', end='2019-01-26', freq='15T')
...
plt.plot(df.index, P_PV_export_grid_opt_flex, marker='o', label='PV_export_grid_optimum_flow')
