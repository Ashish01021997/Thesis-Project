...
for i in range(iterations):
    ...
    model_flex.addConstr(P_bat_export <= abs(P_pv_positive_flex[i])) #R3

    model_flex.addConstr((P_grid_delta_net_flex) <= (P_grid_max))  #R4 block i
    model_flex.addConstr((P_grid_delta_net_flex) >= (P_grid_min)) #R5
    model_flex.addConstr((E_bat_loop_flex) >= (E_bat_min)) #R6
    model_flex.addConstr((E_bat_loop_flex) <= (E_bat_max)) #R7
    model_flex.addConstr(P_bat_charging_PV_flex + P_PV_export_grid_flex <= PV_forecasts[i]) #R8

    model_flex.addConstr(
        ((P_bat_charging_flex + P_grid_import_flex) - (P_bat_discharging_flex + P_PV_export_grid_flex)) == (
                Load_forecasts[i] - PV_forecasts[i])) #R9
    ...
    if model_flex.status == gp.GRB.OPTIMAL:
        ...
        P_bat_charging_PV_opt_flex[i] = P_bat_charging_PV_flex.x + P_pv_negative_flex[i] - P_pv_positive_flex[i]
        ...
        P_bat_charging_opt_flex[i] = P_bat_charging_PV_flex.x + P_pv_negative_flex[i] - P_pv_positive_flex[i]  + P_bat_charging_grid_flex.x - P_bat_positive_flex[i]
...
excel_file_path = "Output_Prosumer_Cost_PV_Pos_Flex.xlsx"  # Specify the path where you want to save the Excel file
...
plt.title('EMS Deterministic Optimization with positive PV flexibility provision')
plt.xlabel('Time')
...
print("Flex Price in €/ kWh is", Price_flex)
