...
P_grid_max = 3000
P_grid_min = -3000 # GRID LIMITS IN -1.5 kW AND 1.5 kW
...
bat_operational_cost = 0.1248*1000 # 12.48 CENTS/ kWh
...
binary_array = np.zeros(iterations)
...
for i in range(iterations):
   ...
   for j in range(number_of_scenarios_PV):
    ...
    for k in range(number_of_scenarios_Load):
      ...
      P_bat_discharging_flex = (
              P_bat_discharging_load_flex + P_bat_discharging_grid_flex - P_bat_export + P_bat_negative_flex[i])

      battery_cycles = (P_bat_charging_flex + P_bat_discharging_flex) / P_bat_delta

      OBJECTIVE_FUNCTION = ((P_grid_import_flex + P_bat_charging_grid_flex - P_bat_positive_flex[i]) *
                            epex_spot_import_price_flex[i] - (
                                        P_PV_export_grid_flex - P_pv_negative_flex[i] + P_pv_positive_flex[
                                    i] + P_bat_discharging_grid_flex + P_bat_negative_flex[i]) *
                            epex_spot_export_price_flex[
      ...
      if binary_battery_variable == 0:
          ...
          model_flex.addConstr(P_bat_charging_flex == 0) #R0
      ...
      model_flex.addConstr(P_bat_discharging_flex >= P_bat_min) #R3
      model_flex.addConstr(P_bat_discharging_flex <= P_bat_max) #R4
      ...
      model_flex.addConstr(E_bat_loop >= E_bat_min)  #R5
      model_flex.addConstr(E_bat_loop <= E_bat_max)  #R6


      model_flex.addConstr(P_bat_export <= abs(P_bat_negative_flex[i])) #R7
      ...
      model_flex.addConstr(
          ((P_bat_charging_flex + P_grid_import_flex) - (P_bat_discharging_flex + P_PV_export_grid_flex)) == (Load_quantile - PV_quantile)) #R8

      model_flex.addConstr((P_PV_export_grid_flex + P_bat_charging_PV_flex) <= PV_quantile) #R9
...
excel_file_path = "Prosumer_Cost_Battery_Negative_Flexibility_Output.xlsx"  # Specify the path where you want to save the Excel file
...
plt.title('EMS Stochastic Optimization with Battery Negative flexibility provision')
plt.xlabel('Time')
...
for i in range(iterations):
  ...
  cost_per_cycle = bat_operational_cost  # TBD
  ...
  def Bat_Neg_Flex_Price():
          ...
          C_bat_op_neg_flex = 0.5 * cost_per_cycle / n_cycles * abs(P_bat_discharging_opt_flex[i]) / P_bat_max

          expenses = -abs(P_bat_discharging_grid_opt_flex[i] - P_bat_discharging_grid_opt[i]) * epex_spot_export_price_flex[i] * time_res

          price_flex_bat_neg = (expenses * (1 + RM) + C_bat_op_neg_flex) / abs(E_bat_negative_flex[i])
...
# code to plot the positive and negative battery flexibilities

time_res = 15/60

E_battery_grid_optimum = np.zeros(iterations)
E_battery_grid_optimum[0] = E_bat_initial/1000
E_battery_grid_optimum_flex = np.zeros(iterations)
E_battery_grid_optimum_flex[0] = E_bat_initial/1000
...
for i in range(iterations):
   ...
   E_battery_grid_optimum_flex[i] = E_battery_grid_optimum_flex[i - 1] + (
           abs(P_bat_charging_grid_opt_flex[i - 1]) - abs(P_bat_discharging_grid_opt_flex[
                                                              i - 1])) * time_res / 1000


   if i <= 20 and i >= 12: # for the Battery flex interval (3:00 till 5:00)
       ...
       time_pos_flex_vector = [datetime(2019, 7, 25, 0, 0) + timedelta(minutes=int(time)) for time in
                               original_time_array]
       ...
       # Convert to datetime objects
       time_neg_flex_vector = [datetime(2019, 7, 25, 0, 0) + timedelta(minutes=int(time)) for time in
                               original_time_array]
...
plt.title('Exchanged Energy in Stochastic Optimization with Negative Battery Flexibility')
