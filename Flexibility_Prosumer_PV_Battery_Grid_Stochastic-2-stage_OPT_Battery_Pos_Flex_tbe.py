...
bat_operational_cost = 124.8 # 1 cent/NO_OF_CHARGING/DISCHARGING_CYCLES
   ...
   def bat_positive_flex():  # battery charge from grid
         ...
         if (E_bat_flex_check[i + 1] - E_bat_final) <= sum(
            ...
            for k in range(i + 1, i + 9, 1):
                 ...
                 if abs(P_bat_max - abs(P_bat_charging_grid_opt[k])) >= abs(P_bat_max - abs(P_bat_charging_grid_opt[i])):
...
for i in range(iterations):
 ...
 cost_per_cycle = bat_operational_cost  # 5  cents is the cost per battery charging/discharging cycle
 ...
 def Bat_Pos_Flex_Price():
        ...
        savings_bat_pos_flex = abs(- P_bat_charging_grid_opt[i] + P_bat_charging_grid_opt_flex[i]) * epex_spot_import_price_flex[i] * time_res
