...
bat_operational_cost = 124.8# 10 cent/NO_OF_CHARGING/DISCHARGING_CYCLES
...
for i in range(iterations):
   ...
   for j in range(number_of_scenarios_PV):
    ...
    for k in range(number_of_scenarios_Load):
      ...
      model_flex.addConstr(P_bat_export <= -P_pv_negative_flex[i])
      ...
      if model_flex.status == gp.GRB.OPTIMAL:
        ...
        P_PV_export_grid_scenario_profile[l] = -abs(P_PV_export_grid_flex.x + P_pv_positive_flex[i] - P_pv_negative_flex[i])
...
plt.title('EMS Stochastic Optimization with PV negative flexibility provision')
plt.xlabel('Time')
...
print("PV Positive Flex Price  in €/ kWh is", PV_Positive_Flex_Price)
