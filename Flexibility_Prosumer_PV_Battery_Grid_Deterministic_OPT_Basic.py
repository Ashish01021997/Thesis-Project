...
class EMS_Basic_Deterministic_Optimization:
 ...
 P_bat_charging_opt_effective = np.zeros(iterations)

 P_bat_discharging_opt = np.zeros(iterations)

 P_bat_discharging_opt_effective = np.zeros(iterations)
 ...
 for i in range(iterations):
    ...
    model.addConstr(
           ((P_bat_charging + P_grid_import) - (P_bat_discharging + P_PV_export_grid)) == (Load_forecasts[i] - PV_forecasts[i]))
 ...
 plt.title('EMS Deterministic Basic Optimization without flexibility provision')
 plt.xlabel('Time')
