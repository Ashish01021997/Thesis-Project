...
Prosumer_Cost = np.zeros(iterations)
...
for i in range(iterations):
    ...
    P_bat_discharging_flex = (
            P_bat_discharging_load_flex + P_bat_discharging_grid_flex + P_bat_negative_flex[i] - P_bat_export)

    if binary_battery_variable == 0:# Charging happens


        model_flex.addConstr(P_bat_discharging_flex == 0)

    else: # Discharging happens
    ...
    model_flex.addConstr(P_bat_export <= abs(P_bat_negative_flex[i])) #R3/4

    model_flex.addConstr((P_grid_delta_net_flex) <= (P_grid_max))  # R4/5
    model_flex.addConstr((P_grid_delta_net_flex) >= (P_grid_min)) # R5/6
    model_flex.addConstr((E_bat_loop_flex) >= (E_bat_min)) #R6/7
    model_flex.addConstr((E_bat_loop_flex) <= (E_bat_max)) #R7/8
    model_flex.addConstr(P_bat_charging_PV_flex + P_PV_export_grid_flex <= PV_forecasts[i]) #R8/9

    model_flex.addConstr(
        ((P_bat_charging_flex + P_grid_import_flex) - ( P_bat_discharging_flex + P_PV_export_grid_flex)) == (
...
df = pd.DataFrame(Prosumer_Cost)

# Write DataFrame to Excel file
excel_file_path = "Output_Prosumer_Cost_Bat_Neg_Flex.xlsx"  # Specify the path where you want to save the Excel file
...
plt.title('EMS Deterministic Optimization with negative Battery flexibility provision')
plt.xlabel('Time')
...
for i in range(iterations):
 ...
 cost_per_cycle = bat_operational_cost # 1 cents is the cost per battery charging/discharging

 n_cycles = (P_bat_charging_opt_flex[i] + P_bat_discharging_opt_flex[i])/ P_bat_delta  # to be found
 ...
 def Bat_Neg_Flex_Price():
        ...
        expenses = -abs(P_bat_discharging_grid_opt_flex[i] - P_bat_discharging_grid_opt[i]) * epex_spot_export_price_flex[i] * time_res # profit
...
print("Flex Price in Euro/kWh is", Price_flex)
