...
class Scenario_Generatiom_Reduction_PV_Quantiles():

 PV_Quantiles = Quantile_PV_forecasts.PV_Quantiles()

 df_predictions_PV = PV_Quantiles.df_predictions_PV
 ...
 timescale = np.arange(0, 672, 1)

 PV_scenarios = np.zeros(96)
...
# Producing Excel file with results of reduced PV scenarios

   for i in range(96):

       PV_scenarios[i] = df_reduced_scenarios.iloc[i, 1] # taking the 2nd scenario

   df_pv_scenarios = pd.DataFrame(PV_scenarios)

   excel_file_path = "Solar_PV_scenarios_output.xlsx"  # Specify the path where you want to save the Excel file
   df_pv_scenarios.to_excel(excel_file_path, index=False)
...
# Display results
 ...
 plt.title('Original Scenarios Quantile Regression PV Forecasts')
 plt.xlabel('X')
 plt.ylabel('y')
 plt.legend()
 plt.show()
 ...
 plt.title('Reduced Scenarios Quantile Regression PV Forecasts')
 plt.xlabel('X')
 plt.ylabel('y')
 plt.legend()
 plt.show()
