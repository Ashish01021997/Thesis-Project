...
P_grid_max = 3000
P_grid_min = -3000 # GRID LIMITS IN 0.1 kW AND 4 kW
...
bat_operational_cost = 0.1248*1000 # 10 cent/NO_OF_CHARGING/DISCHARGING_CYCLES
...
P_PV_export_grid_profile = Flex_Basic_Optimization.P_PV_export_grid_profile
...
P_grid_import_profile = Flex_Basic_Optimization.P_grid_import_profile
...
P_bat_charging_scenario_opt_effective = np.zeros(number_of_scenarios)

P_bat_discharging_scenario_opt = np.zeros(number_of_scenarios)

P_bat_discharging_scenario_opt_effective = np.zeros(number_of_scenarios)
...
for i in range(iterations):
   ...
   def PV_positive_flex():  # curtailing pv feed in to grid
         ...
         P_pv_positive_flex[i] = -abs(P_PV_export_grid_profile[i])
         ...
         for j in range(i + 1, i + 9, 1):
             ...
             if abs(P_PV_export_grid_profile[j]) >= abs(P_pv_positive_flex[i]):
   ...
   if i == 47:  # 12:00  PM PV +ve  Flexibility Check
   ...
   epex_spot_import_price_flex[i] = (EM_Price_15_min_res[i])

   epex_spot_export_price_flex[i] = 90
   ...
   for j in range(number_of_scenarios_PV):
    ...
    for k in range(number_of_scenarios_Load):
      ...
      OBJECTIVE_FUNCTION = ((P_grid_import_flex + P_bat_charging_grid_flex - P_bat_positive_flex[i]) * epex_spot_import_price_flex[i] - (P_PV_export_grid_flex - P_pv_negative_flex[i] + P_pv_positive_flex[i] + P_bat_discharging_grid_flex + P_bat_negative_flex[i]) * epex_spot_export_price_flex[
                      i])*time_res
      ...
      model_flex.addConstr(P_bat_charging_flex >= P_bat_min) #1
      model_flex.addConstr(P_bat_charging_flex <= P_bat_max) #2

      model_flex.addConstr(P_bat_discharging_flex >= P_bat_min) #1
      model_flex.addConstr(P_bat_discharging_flex <= P_bat_max) #2
      ...
      E_bat_loop = E_bat_loop_current_scenario[l] + (P_bat_charging_flex * eta_batt - P_bat_discharging_flex / eta_batt) * (time_res)  # time_res is 15 min

      model_flex.addConstr(P_grid_delta_net <= P_grid_max)  # 3/4
      model_flex.addConstr(P_grid_delta_net >= P_grid_min)  # 4/5
      model_flex.addConstr(E_bat_loop >= E_bat_min)  # 5/6
      model_flex.addConstr(E_bat_loop <= E_bat_max)  # 6/7

      if i == (iterations - 1):
          model_flex.addConstr(E_bat_loop == E_bat_final)

      model_flex.addConstr(P_bat_export <= -P_pv_positive_flex[i])
      ...
      if model_flex.status == gp.GRB.OPTIMAL:
        ...
        Prosumer_Cost[i] += model_flex.objVal * average_stochastic_probabilities[l]
        ...
        E_bat_scenario_profile[l] = E_bat_loop_current_scenario[l] + (((P_bat_charging_PV_flex.x + P_pv_negative_flex[i]  + P_bat_charging_grid_flex.x - P_bat_positive_flex[i]) * eta_batt - (
        ...
        P_bat_charging_PV_scenario_opt[l] = P_bat_charging_PV_flex.x - P_pv_positive_flex[i] + P_pv_negative_flex[i]
        ...
        P_bat_charging_opt_flex_effective[i] += P_bat_charging_scenario_opt_effective[l] * \
                                                average_stochastic_probabilities[
                                                    l]
        P_bat_discharging_opt_flex_effective[i] += P_bat_discharging_scenario_opt_effective[l] * \
                                                   average_stochastic_probabilities[l]
...
df_prosumer_cost = pd.DataFrame(Prosumer_Cost)

excel_file_path = "Prosumer_Cost_PV_Positive_Flexibility_Output.xlsx"  # Specify the path where you want to save the Excel file

df_prosumer_cost.to_excel(excel_file_path, index=False)
...
plt.plot(time_points, P_grid_import_profile_flex, marker='v', label='P_grid_import_optimum_flow')
plt.plot(time_points, P_PV_export_grid_profile_flex, marker='o', label='PV_export_grid_optimum_flow')
...
plt.title('EMS Stochastic Optimization with PV positive flexibility provision')
plt.xlabel('Time')
...
for i in range(iterations):
 ...
 cost_per_cycle = bat_operational_cost  # 1 cents is the cost per battery charging/discharging cycle
 ...
 def Bat_Neg_Flex_Price():
        ...
        expenses = ((P_grid_import_opt_flex[i] + P_bat_charging_grid_opt_flex[i] - P_grid_import_profile[i] - P_bat_charging_grid_opt[i]) * epex_spot_import_price_flex[i] +
 ...
 def PV_Pos_Flex_Price():
        ...
        losses = abs(P_PV_export_grid_profile[i] - P_PV_export_grid_profile_flex[i]) * epex_spot_export_price_flex[i] * time_res
 ...
 for k in range(len(pv_pos_flex_iteration_list)):
    ...
    if i == pv_pos_flex_iteration_list[k]:
      ...
      PV_Positive_Flex_Price[i] = PV_Pos_Flex_Price()
...
print("Battery Negative Flex Price in ", Bat_Negative_Flex_Price)

print("PV Positive Flex Price  in €/ kWh is", PV_Positive_Flex_Price/1000)
...
E_PV_grid_optimum[0] = E_bat_initial
E_PV_grid_optimum_flex = np.zeros(iterations)
E_PV_grid_optimum_flex[0] = E_bat_initial
...
for i in range(iterations):
   ...
   E_PV_grid_optimum[i] = E_PV_grid_optimum[i - 1] + (P_PV_export_grid_profile[i - 1]) * time_res / 1000

   E_PV_grid_optimum_flex[i] = E_PV_grid_optimum_flex[i - 1] + (P_PV_export_grid_profile_flex[i - 1]) * time_res / 1000

   if i <= 55 and i >= 47: # for the PV flex interval (12:00 till 14:00)
...
plt.ylabel('Exchanged Energy in kWh')
