...
bat_operational_cost = 0.1248*1000 # Prices are in EUR/MWh and Bat operational cost is 12.48 cents/kWh
...
for i in range(iterations):
    ...
    if i == 12  and counter_bat_negative_flex == 0:# 3:15 AM Battery -ve  Flexibility Check
    ...
    P_bat_discharging_flex = (P_bat_discharging_load_flex + P_bat_discharging_grid_flex - P_bat_export + P_bat_negative_flex[i])
    ...
    model_flex.addConstr(P_bat_export <= abs(P_bat_negative_flex[i])) #R10
    ...
    BAT_FLEX_OBJ = ((P_grid_import_flex + P_bat_charging_grid_flex - P_bat_positive_flex[i]) * epex_spot_import_price_flex[i] - (P_PV_export_grid_flex - P_pv_negative_flex[i] + P_pv_positive_flex[i] + P_bat_discharging_grid_flex + P_bat_negative_flex[i]) * epex_spot_export_price_flex[
                      i])*time_res + battery_cycles_flex*bat_operational_cost
    ...
    if model_flex.status == gp.GRB.OPTIMAL:
        ...
        Prosumer_Cost[i] = model_flex.objVal
        ...
        P_PV_export_grid_opt_flex[i] = -(P_PV_export_grid_flex.x + P_pv_positive_flex[i] - P_pv_negative_flex[i])
        ...
        P_bat_charging_opt_flex[i] = P_bat_charging_PV_flex.x + P_pv_negative_flex[i] + P_bat_charging_grid_flex.x - P_bat_positive_flex[i] - P_pv_positive_flex[i]
...
excel_file_path = "Prosumer_Cost_Battery_Negative.xlsx"  # Specify the path where you want to save the Excel file
...
plt.title('EMS Robust Optimization with negative Battery flexibility provision')
plt.xlabel('Time')
...
for i in range(iterations):
 ...
 cost_per_cycle = bat_operational_cost   #
...
print(P_bat_discharging_grid_opt_flex)
...
plt.title('Comparison of Exchanged Energy in Basic and Flexible Robust Optimization')
