...
class EMS_Basic_Optimization:
 ...
 bat_operational_cost = 0.1248*1000 # 1 cent/NO_OF_CHARGING/DISCHARGING_CYCLES
 ...
 binary_opt = np.zeros(iterations*number_of_scenarios+1)
 ...
 for i in range(iterations):
  ...
  for j in range(number_of_scenarios_PV):
   ...
   for k in range(number_of_scenarios_Load):
    ...
    epex_spot_export_price[i] = 0.09*1000  # feed-in-tarif is fixed at 9 cents/kWh
    ...
    OBJECTIVE_FUNCTION = ((P_grid_import + P_bat_charging_grid) * epex_spot_import_price[i] - (P_PV_export_grid+ P_bat_discharging_grid) * epex_spot_export_price[i])*time_res + battery_cycles*bat_operational_cost

    model.setObjective(OBJECTIVE_FUNCTION, gp.GRB.MINIMIZE)

    # Battery max and min power constraints with  charging or discharging events (1ST STAGE CONSTRAINTS)
    ...
    model.addConstr(
        ((P_bat_charging + P_grid_import) - (P_bat_discharging + P_PV_export_grid)) == (Load_quantile - PV_quantile)) #R8
 ...
 #plt.xlabel('Time')
 #plt.ylabel('Value')
 plt.title('EMS Stochastic Optimization without flexibility provision')
 plt.xlabel('Time')
 ...
 # code to plot the optimum battery-grid exchanged energy without flexibility
